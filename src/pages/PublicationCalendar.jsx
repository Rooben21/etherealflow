...
export default function PublicationCalendar(){
 ...
 return <div className="space-y-5">
  ...
  <Link to="/campaigns" className="studio-button">Контент на місяць — кампанії та сценарії</Link>
  ...
  <p className="text-xs text-muted-foreground leading-relaxed">Часовий пояс: {Intl.DateTimeFormat().resolvedOptions().timeZone} · {isLoading?'Завантаження планів…':error?'Не вдалося отримати плани':`Публікацій: ${rows.filter(item=>item._kind!=='campaign'&&isSameMonth(new Date(item.scheduled_at),month)).length} · Відеопланів: ${rows.filter(item=>item._kind==='campaign'&&isSameMonth(new Date(item.scheduled_at),month)).length}`}<span className="block mt-1">Оберіть день для деталей. Чернетки та «публікувати зараз» показані за датою постановки в чергу.</span></p>
