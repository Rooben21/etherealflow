...
export default function useStudioData() {
 ...
 const operations=useQuery({queryKey:['studio-operations',month],queryFn:()=>base44.entities.StudioOperation.filter({created_date:{$gte:month}},'-created_date',500)});
 const refresh=()=>Promise.all(['studio-videos','studio-settings','studio-operations','studio-video','studio-assets'].map(key=>qc.invalidateQueries({queryKey:[key]})));
