...
export async function providerRequest(provider, path, body) {
  ...
  let response;
  try {
    response = await fetch(base + path, { method: body === undefined ? 'GET' : 'POST', headers: { Authorization: provider === 'postiz' ? key : `Bearer ${key}`, ...(body === undefined ? {} : {'Content-Type':'application/json'}) }, ...(body === undefined ? {} : {body:JSON.stringify(body)}), redirect:'manual', signal:AbortSignal.timeout(45000) });
...
export async function connectedAccounts() {
  const rows = await providerRequest('postiz','/integrations');
