...
export default async function(req){
 ...
 try{
  ...
  if(open){if(open.state==='result'||open.result){await applyCampaignStep(client,c,open);return Response.json({ok:true,recovered:true});}await client.entities.ContentCampaign.update(c.id,{status:'needs_attention',message:'Незавершений платний запит. Автоматичний повтор заблоковано; відновіть крок.'});return Response.json({ok:true,needsAttention:true});}
  ...
  studio=await takeStudioLock(client,claim.token);const usage=await operationUsage(client,{campaignId:c.id}),estimate=1;
  ...
  const current=await client.entities.ContentCampaign.get(c.id);if(current.status!=='running'||current.worker_token!==claim.token)return Response.json({ok:true,paused:true});
  ...
  if((await client.entities.ContentCampaign.get(c.id)).worker_token!==claim.token)return Response.json({ok:true,resultSaved:true,needsRecovery:true});
