...
export default async function(req) {
 try {
  ...
  if(typeof id!=='string' || id.length>100 || !['asset','video'].includes(kind)) return Response.json({error:'Некоректний запит.'},{status:400});
  const record = kind==='asset' ? await client.entities.StudioAsset.get(id) : await client.entities.StudioVideo.get(id);
